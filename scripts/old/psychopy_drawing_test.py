        ...
        for i_trial,(flickering_freq, phase_offset) in enumerate(sequence): # for each trial in the trail sequence
            ...
            for thisKey in keys:
                if thisKey=='escape':
                    ...
                    core.quit()
            for frame in range(ms_to_frame(isi_duration*1000, refresh_rate)):
                for i_key,key in enumerate(flickering_keyboard2):
                    # if i_key == class_num:
                    #     key.color = (1,1,1)
                    # else:
                    #     key.color = (-1,-1,-1)
            ...
            while(not flash_successful):
                ...
                for i_frame,frame in iter_frame:
                    ...
                    # for key in current_keyboard:
                        ...
                        # key.color = (key_frame,key_frame,key_frame)
                        key.draw()
                        # if core.getTime() > (next_flip - 0.000):
                        #     break
                    # if core.getTime() < (next_flip - 0.000):
                    # if core.getTime() > (next_flip + 0.002):
                    if core.getTime() > next_flip:
                        ...
                        for failure_frame in range(15):
                            for i_key,key in enumerate(flickering_keyboard3):
