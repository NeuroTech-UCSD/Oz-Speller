...
run_count = 0
first_call = True
time_stamp = multiprocessing.Value('d', 0)
...
def ExampleSampleCallback_Signals( headsetPtr, packetTime, userData ):
    ...
    run_count += 1
    ctypes.cast(userData, ctypes.py_object).value.value = packetTime
...
if __name__ == "__main__": 
    # print(id(time_stamp))
    recording = multiprocessing.Process(target=record,daemon=True,args=(time_stamp,))
    recording.start()
    time.sleep(6)
        ...
        for frame in range(ms_to_frame(750, refresh_rate)):
            if frame == 0:
                with open("meta.csv", 'a') as csv_file:
                    csv_file.write(str(flickering_freq) + ', ' + str(time_stamp.value) + '\n')
            fixation.draw()
            win.flip()
